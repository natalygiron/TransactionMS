...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiError> badInput(ServerWebInputException ex, ServerWebExchange exchange) {
    return build(HttpStatus.BAD_REQUEST, "JSON/params inválidos o con formato incorrecto", exchange,
        null);
  }

  // ⇣ Errores hacia AccountMS vía WebClient
  @ExceptionHandler(WebClientResponseException.class)
  public ResponseEntity<ApiError> downstream(WebClientResponseException ex,
      ServerWebExchange exchange) {
    HttpStatus status = HttpStatus.valueOf(ex.getStatusCode().value());
    String msg = status.is4xxClientError() ? "Error al aplicar operación en AccountMS"
