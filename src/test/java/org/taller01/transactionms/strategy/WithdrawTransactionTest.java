...
class WithdrawTransactionTest {
    ...
    void setUp() {
        ...
        withdrawTransaction = new WithdrawTransaction(repo, accountClient, factory);
    }

    @Test
    void execute_success() {
        WithdrawRequest req = new WithdrawRequest("acc1", BigDecimal.valueOf(50));
        Transaction tx = factory.success(TransactionType.WITHDRAWAL, "acc1", null, req.amount(), Messages.WITHDRAW_SUCCESS);

        when(accountClient.withdraw(any(), any())).thenReturn(Mono.empty());
        when(repo.save(any(Transaction.class))).thenReturn(Mono.just(tx));

        StepVerifier.create(withdrawTransaction.execute(req))
                .expectNextMatches(t -> t.getStatus().name().equals("SUCCESS"))
                .verifyComplete();

        verify(accountClient).withdraw("acc1", req.amount());
